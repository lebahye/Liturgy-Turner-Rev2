...
Real-time audio processing for automatic liturgy page turning. This Clawdbot skill listens to the deacon/priest during Armenian liturgy, transcribes what is spoken, matches it against a liturgy phrase database, and calls your Liturgy Turner API to flip the on-screen page automatically.
...
| `liturgy status` | Show current page, config, and training info |
| `next page` / `previous page` | Manual page control |
| `go to page 42` | Jump directly to a page |
| `start training` | Enable training mode (stores audio/transcript pairs) |
| `this is page 10` | Tag the current audio as page 10 |
...
```
Add as many entries as necessary for your liturgy book and phrases your clergy use often.

## Requirements

- Microphone access on the machine running Clawdbot (WSL users may prefer a USB mic passed through).
- OpenAI API key configured for the embedded gateway so the skill can call Whisper.
...
- **No audio detected**: confirm `mic` and `node-record-lpcm16` can access your device; test with `arecord -l`.
